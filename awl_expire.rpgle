     ...
     C* Expire records.
     C/EXEC SQL
     C+ DELETE FROM spamassass/awl WHERE (
     ...
     C+ SELECT COUNT(*) FROM spamassass/awl
     C/END-EXEC
     C*
     C* Get the count of expirable records.
     C/EXEC SQL DECLARE SLT2 CURSOR FOR
     C+ SELECT COUNT(*) FROM spamassass/awl WHERE (
     C+  count=1 AND TIMESTAMP(last_hit) < (CURRENT_TIMESTAMP - 7 DAYS)
     C+ ) or (
     C+  TIMESTAMP(last_hit) < (CURRENT_TIMESTAMP - 2 MONTHS)
     C+ )
     ...
     C/END-EXEC
     C*
     C                   CALLP     Qp0zLprintf('AWL-Cleanup: %s entries, +
