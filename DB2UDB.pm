...
Mail::SpamAssassin::BayesStore::DB2UDB - IBM OS/400 DB/2 SQL Specific Bayesian Storage Module Implementation

=head1 DESCRIPTION

This module implements an IBM OS/400 DB/2 specific based bayesian storage
module. It requires that you are running at least version V4R5 of OS/400, and
properly set up an ODBC connection to the machine.
...
package Mail::SpamAssassin::BayesStore::DB2UDB;
...
public class (Mail::SpamAssassin::BayesStore::DB2UDB) new (Mail::Spamassassin::Plugin::Bayes $bayes)
...
sub calculate_expire_delta {
  ...
  for (my $i = 1; $i <= $max_expire_mult; $i<<=1) {
    $sth->execute($self->{_userid}, $newest_atime - $start * $i);
...
sub token_expiration {
  ...
  $self->{_dbh}->do($sql, undef, $vars[10], $self->{_userid}, $vars[10]);
  ...
  # Sanity check: if we're about to expire too many tokens, abort!
  ...
  $self->{_dbh}->do($sql, undef, $deleted, time(), $newdelta, $deleted, $self->{_userid});
  ...
  if ($deleted) {
    ...
    $self->{_dbh}->do($sql, undef, $oldest_token_age, $self->{_userid});
...
sub seen_put {
  ...
  $self->{_dbh}->do($sql, undef, $self->{_userid}, $msgid, $flag);
...
sub seen_delete {
  ...
  $self->{_dbh}->do($sql, undef, $self->{_userid}, $msgid);
...
sub set_last_expire {
  ...
  $self->{_dbh}->do($sql, undef, $time, $self->{_userid});
...
sub set_running_expire_tok {
  ...
  $self->{_dbh}->do($sql, undef, $self->{_userid}, $time);
...
sub tok_touch {
  ...
  $sth->execute($self->{_userid}, $token, $atime);
  if (defined($self->{_dbh}->errstr())) {
    dbg("bayes: tok_touch: SQL error: ".$self->{_dbh}->errstr());
    return 0;
  }
  ...
  $self->{_dbh}->do($sql, undef, $atime, $self->{_userid}, $token, $atime);
...
sub cleanup {
  ...
  my ($toks_delete) = $sth->fetchrow_array();
  ...
  $sql = "UPDATE bayes_vars SET token_count = token_count - $toks_delete
           WHERE id = ?";
...
sub clear_database {
  ...
  $self->{_dbh}->do("DELETE FROM bayes_vars WHERE id = ?",
			       undef,
			       $self->{_userid});
  ...
  $self->{_dbh}->do("DELETE FROM bayes_seen WHERE id = ?",
			    undef,
			    $self->{_userid});
  if (defined($self->{_dbh}->errstr())) {
  ...
  $self->{_dbh}->do("DELETE FROM bayes_token WHERE id = ?",
			    undef,
			    $self->{_userid});
  if (defined($self->{_dbh}->errstr())) {
...
sub _initialize_db {
  ...
  $self->{_dbh}->do($sqlinsert,
			       ...
			       $self->{_username});
  if (defined($self->{_dbh}->errstr())) {
    dbg("bayes: _initialize_db (5): SQL error: ".$self->{_dbh}->errstr());
    return 0;
  }
  ...
  if ($self->{_dbh}->errstr()) {
    dbg("bayes: _initialize_db (6): SQL error: ".$self->{_dbh}->errstr());
    return 0;
  }
...
sub _put_token {
  ...
  if (!$existing_atime) {
    ...
    if ($rows eq 0) {
      ...
      $self->{_dbh}->do($sql, undef, $atime, $self->{_userid}, $atime);
      if (defined($self->{_dbh}->errstr())) {
	dbg("bayes: _put_token: SQL error executing update oldest token age: ".$self->{_dbh}->errstr());
	return 0;
      }
    }
  } else {
    ...
    if ($updated_atime_p) {
      ...
      if (defined($self->{_dbh}->errstr())) {
	dbg("bayes: _put_token: SQL error in final update newest token age: ".$self->{_dbh}->errstr());
	return 0;
      }
    }
  }
...
sub _put_tokens {
  ...
  foreach my $token (keys %{$tokens}) {
    ...
    if (!$existing_atime) {
      ...
      $insertsth->execute($self->{_userid}, $token, $spam_count, $ham_count, $atime);
  ...
  if ($new_tokens) {
    ...
    $self->{_dbh}->do($sql, undef, $new_tokens, $self->{_userid});
